...
namespace OrdersMS.Tests.Orders.Application.Commands.UpdateOrderStatusToPaid
{
    public class UpdateOrderStatusToPaidCommandHandlerTests
    {
        private readonly Mock<IOrderRepository> _orderRepositoryMock;
        private readonly Mock<IPublishEndpoint> _publishEndpointMock;
        private readonly UpdateOrderStatusToPaidCommandHandler _handler;

        public UpdateOrderStatusToPaidCommandHandlerTests()
        {
            _orderRepositoryMock = new Mock<IOrderRepository>();
            _publishEndpointMock = new Mock<IPublishEndpoint>();
            _handler = new UpdateOrderStatusToPaidCommandHandler(_orderRepositoryMock.Object, _publishEndpointMock.Object);
        }

        [Fact]
        public async Task ShouldUpdateOrderStatusToPaidSuccess()
        {
            ...
            _publishEndpointMock.Setup(x => x.Publish(It.IsAny<OrderPaidEvent>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
            ...
            _publishEndpointMock.Verify(x => x.Publish(It.IsAny<OrderPaidEvent>(), It.IsAny<CancellationToken>()), Times.Once);
        }
        ...
        [Fact]
        public async Task ShouldFailToUpdateOrderStatusWhenUpdateFails()
        {
            var order = Order.CreateOrder(
                new OrderId("53c0d8fa-dbca-4d98-9fdf-1d1413e90d8a"),
                new ContractId("53c0d8fa-dbca-4d98-9fdf-1d1413e90f0d"),
        ...
        [Fact]
        public async Task ShouldFailToUpdateOrderWhenOrderStatusIsDifferentToCompleted()
        {
            var order = Order.CreateOrder(
                new OrderId("53c0d8fa-dbca-4d98-9fdf-1d1413e90d8a"),
                new ContractId("53c0d8fa-dbca-4d98-9fdf-1d1413e90f0d"),
            ...
            _orderRepositoryMock.Setup(x => x.GetById(order.GetId())).ReturnsAsync(Optional<Order>.Of(order));
            _orderRepositoryMock.Setup(x => x.Update(order)).ReturnsAsync(Result<Order>.Failure(new OrderUpdateFailedException("The order is not in the Finalizado status")));

            var result = await _handler.Execute((order.GetId(), command));

            Assert.NotNull(result);
            Assert.False(result.IsSuccessful);
            Assert.Equal("The order is not in the Finalizado status", result.ErrorMessage);
        }
    }
}